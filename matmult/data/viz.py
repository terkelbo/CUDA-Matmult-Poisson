...
df6.plot('memory', ax=ax, style='*-')
df7.plot('memory', ax=ax, style='*-')
plt.legend(["GPU1","GPU2","GPU3","GPU4","GPU5",'CPU','cublasDgemm'],loc='upper left')
plt.xlabel('Memory Footprint (Mbyte)')
plt.ylabel('Performance (Gflop/s)')
#plt.gca().set_ylim(bottom=0)
plt.xscale('log',basex=2)
...
ax.set_major_formatter(FormatStrFormatter('%.2f'))
plt.xticks(rotation=45)
plt.savefig('Mflops_matmul_zoom.png', bbox_inches='tight')
plt.close()	                     																		
...
df6.plot('memory', ax=ax, style='*-')
plt.legend(["GPU3","CPU"],loc='upper left')
plt.xlabel('Memory Footprint (Mbyte)')
plt.ylabel('Performance (Gflop/s)')
#plt.gca().set_ylim(bottom=0)
plt.xscale('log',basex=2)
