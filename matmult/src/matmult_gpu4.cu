...
void matmult_gpu4(int m, int n, int k, double * A, double * B, double * C){
	...
	//kernel block and grid size
    dim3 dimBlock(16,16,1);
    dim3 dimGrid((int)ceil(((double)m)/(16*REGISTER_BLOCKING)), (int)ceil(((double)n)/(16)));  
...
__global__ void matmult_gpu4Kernel(int m, int n, int k, double * d_A, double * d_B, double * d_C){
    ...
    i = REGISTER_BLOCKING*(blockIdx.x * blockDim.x + threadIdx.x);
...
*/
// REGISTER BLOCKING ALONG THE ROWS & COLUMNS OF C (SQUARE)
/*
extern "C" {
void matmult_gpu4(int m, int n, int k, double * A, double * B, double * C){
	double * d_A, * d_B, * d_C;
	...
	//kernel block and grid size
    dim3 dimBlock(16,16,1);
    dim3 dimGrid((int)ceil(((double)m)/(16*REGISTER_BLOCKING)), (int)ceil(((double)n)/(16*REGISTER_BLOCKING)));  
...
__global__ void matmult_gpu4Kernel(int m, int n, int k, double * d_A, double * d_B, double * d_C){
	...
	if(i < m && j < n){
		for(l=0;l < k;l++){
			C_reg[0] += d_A[i*k + l] * d_B[l*n + j];
			for(ei = 1; ei < REGISTER_BLOCKING; ei++){
				for(ej = 1; ej < REGISTER_BLOCKING; ej++){			
					if(i + ei < m && j + ej < )
						C_reg[e] += d_A[(i+e)*k + l] * d_B[l*n + j + e];	
				}
			}
		}
		d_C[i*n + j] = C_reg[0];
